...
const IndustryCard = ({ industry }: { industry: typeof industries[0] }) => {
  return (
    ...
    >
      {/* Removed gradient overlay since we now have a dark background behind text */}
      <img
        src={industry.image}
        alt={`${industry.title} privacy solutions`}
        className="w-full h-48 object-cover transition-transform group-hover:scale-105"
      />
      <div className="absolute bottom-0 left-0 right-0 p-3 z-20 bg-black bg-opacity-50">
        <h3 className="text-lg font-bold text-[#3674B5] mb-1">{industry.title}</h3>
        <p className="text-white text-opacity-90 text-xs mb-2">
          {industry.description}
        </p>
        <a href="#" className="text-[#3674B5] text-xs inline-flex items-center font-medium">
          Learn more
